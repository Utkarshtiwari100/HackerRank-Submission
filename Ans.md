...
if(y2 == y1){
if(m1 == m2) return 15 _ (d1 - d2);
else return (m1 - m2) _ 500;
}
else return 10000;
return 0;
}
...
int main() {
/_ Enter your code here. Read input from STDIN. Print output to STDOUT _/  
 int n;
cin >> n;
vector<int> vec(n);
...
vector<int> result;
for(int i = min(a, b) _ (n-1); i < max(a,b) _ (n-1); i += abs(b - a)) result.push_back(i);
